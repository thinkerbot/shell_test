...
ShellTest uses a pseudoterminal (pty) to execute shell commands and capture
output. Typically tests are constructed to exit the pty shell and wait for an
EOF, which works almost all the time. Occasionally the shell will not provide an
EOF on the pty slave and as a result the test will time-out with an error:

  test_assert_script_match_matches_regexps_to_output(ShellMethodsTest):
  ShellTest::ShellMethods::Agent::UnsatisfiedError: timeout waiting for EOF
...
Clearly the shell is signaling exit, but no EOF is found. This unfortunate error
crops up rarely, but apparently at random, and for apparently random tests.
